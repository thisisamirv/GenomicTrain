...
## FASTA file
Now let's check a FASTA file:
```{bash}
head -n 1000 Reference/Homo_sapiens.GRCh38.dna.chromosome.1.fa | tail -5 | less -S
```
...
```{bash}
head -n 1000 Reference/Homo_sapiens.GRCh38.dna.chromosome.1.fa | tail -10 | less -S
```

2- Prepare the reference genome annotation (gtf) file (only chromosome 1 again). Take a peak:
```{bash}
head -n 1 Reference/Homo_sapiens.GRCh38.107.chrom1.gtf | head -c 41 | less -S
```
