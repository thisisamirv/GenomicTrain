...
### Scenario 4
This is an analysis of 22 RNA-seq samples of different human tissues.
```{r}
...
# Take a peak
counts[1:5, c(1, 2, 15, 16)]
```

Let's plot gene counts:
```{r}
plot(
  sort(counts),
  col = "blue",
...
- DE at gene level: consists of gene or exon counts (edgeR, DESeq, DEXSeq)
- DE across isoforms: includes assembling transcripts (Trinity, Oases, Cufflinks, Scripture)
...
In our RNA-seq data for S1, we only had one sample. Instead, we will import another data with 20 samples (10 controls × 10treatments) using the same 4492 genes. We also import a second file that matches samples to treatment groups and is useful to build design matrices and contrasts.
```{r}
library(edgeR)
